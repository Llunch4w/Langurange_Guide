/**
 * @ Author LuckyQ
 * @ Date   2021-05-25 6:03
 * @ Description 模拟女孩化妆场景下的死锁
 */
package myjava.juc;

public class DeadLokTest {
    public static void main(String[] args){
        Mirror mirror = new Mirror();
        EyeBrush eyeBrush = new EyeBrush();

        new Thread(new DeadLockMakeUp(mirror, eyeBrush, "mirror"), "小红").start();
        new Thread(new DeadLockMakeUp(mirror, eyeBrush, "eyeBrush"), "小花").start();
    }
}
...
class DeadLockMakeUp implements Runnable{
    private Mirror mirror = new Mirror();
    private EyeBrush eyeBrush = new EyeBrush();
    private String choice; // 优先使用镜子还是眼刷

    public DeadLockMakeUp(Mirror mirror, EyeBrush eyeBrush, String choice){
        this.mirror = mirror;
